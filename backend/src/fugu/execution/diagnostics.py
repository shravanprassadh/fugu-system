...
def _classify_error(
    *,
    ...
    if "model" in normalized and (
        "not found" in normalized or "unsupported" in normalized or "unavailable" in normalized
    ):
        return SafeExecutionError(
    ...
    if "provider" in normalized and (
        "unavailable" in normalized or "connection" in normalized or "network" in normalized
    ):
        return SafeExecutionError(
