...
class ProviderCatalogue:
    ...
    def __init__(self, providers: tuple[ProviderDefinition, ...]) -> None:
        ...
        self._model_index = {
            (provider.identifier, model.identifier): model for provider in providers for model in provider.models
        }
    ...
    def model(self, provider_identifier: str, model_identifier: str) -> ModelDefinition:
        ...
        if model is None:
            raise ValueError(f"Model {model_identifier!r} is not available for provider {provider.identifier!r}.")
        return model
